...
const meeting = {
  ...
  mutations: {
    ...
    PLACE_TREE_TWO3(state, placeData) {
      state.placeDataTwo3 = placeData
    },
    PLACE_TREE_TWO4(state, placeData) {
      state.placeDataTwo4 = placeData
    }
  },
  actions: {
    placeTree({ commit }) {
    ...
    placeTreeTwo3({ commit }) {
      placeTreeData({ id: 13 }).then(response => {
        // console.log(response.data)
        this.placeDataTwo = response.data
        // console.log(this.placeData)
        commit('PLACE_TREE_TWO3', this.placeDataTwo)
      })
    },
    placeTreeTwo4({ commit }) {
      placeTreeData({ id: 21 }).then(response => {
        // console.log(response.data)
        this.placeDataTwo = response.data
        // console.log(this.placeData)
        commit('PLACE_TREE_TWO4', this.placeDataTwo)
      })
    }
  }
}
