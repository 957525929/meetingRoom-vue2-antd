    ...
    <a-drawer :visible="visibleModify" title="修改会议室信息" @close="modifyClose" width="600px" placement="right">
      <a-form-model ref="ruleForm" :model="formModify" :rules="rules" :label-col="labelCol" :wrapper-col="wrapperCol">
        ...
        <a-form-model-item label="状态">
          <a-radio-group v-model="formModify.state" :disabled="disabledState" @change="change"> 
            <a-radio value="空闲">
              空闲
            </a-radio>
            <a-radio value="禁用">
...
export default {
  ...
  methods: {
    ...
    Modify(record) {
      ...
      // if (record.meetingRoomState == '0') {
      //   this.handlerModify.push('空闲')
      // }
      // if (record.meetingRoomState == '1') {
      //   this.handlerModify.push('开会中')
      // }
      // if (record.meetingRoomState == '2') {
      //   this.handlerModify.push('禁用')
      // }
      ...
      this.formModify.state = this.handlerModify
      if (this.handlerModify == '1') {
