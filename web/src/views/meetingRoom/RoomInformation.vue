  ...
  <a-card :bordered="false">
    ...
    <div id="dataRoomTable">
      <a-table :dataSource="dataSource" :pagination="ipagination" rowKey="meetingroomId" @change="handleTableChange">
        ...
        <a-table-column title="房间号" data-index="placeName" align="center" key="room">
          <template slot-scope="placeName">
            <span>{{placeName.split('.')[2]}}</span>
          </template>
        </a-table-column>
    ...
    <a-drawer :visible="visibleAdd" title="新增会议室" width="600px" placement="right" @close="addClose">
      <a-form-model ref="ruleForm" :model="formAdd" :rules="rules" :label-col="labelCol" :wrapper-col="wrapperCol">
        <a-form-model-item ref="placeName" label="位置" prop="placeName" placeholder="请选择位置">
          <a-cascader style="width: 410px" :options="selectOptions" v-model="formAdd.placeName" placeholder="请选择位置"
            :display-render="displayRender" @click="handlePlaceTree" />
        </a-form-model-item>
        <a-form-model-item ref="room" label="房间号" prop="room">
          <a-input v-model="room" placeholder="请输入房间号"></a-input>
        </a-form-model-item>
        <a-form-model-item label="状态">
          <a-radio-group v-model="formAdd.meetingRoomState" :disabled="disabledState" @change="onChange">
            <a-radio :value="0">
              空闲
            </a-radio>
            <a-radio :value="2">
    ...
    <a-drawer :visible="visibleModify" title="修改会议室信息" @close="modifyClose" width="600px" placement="right">
      <a-form-model ref="ruleForm" :model="formModify" :rules="rules" :label-col="labelCol" :wrapper-col="wrapperCol">
        ...
        <a-form-model-item label="状态">
          <a-radio-group v-model="formModify.state" :disabled="disabledState" @change="onChange">
            <a-radio :value="0">
              空闲
            </a-radio>
            <a-radio :value="2">
  ...
  export default {
    ...
    methods: {
      ...
      onChange(e) {
        ...
        this.radioRadio = e.target.value
      },
      addRoom() {
      ...
      onSubmitAdd() {
        this.$refs.ruleForm.validate(valid => {
          ...
          this.handleFormAddData.push({
            ...
            whiteboard: this.formAdd.whiteboard || 0,
            blackboard: this.formAdd.blackboard || 0,
            number: this.formAdd.number,
            projector: this.formAdd.projector || 0,
            pc: this.formAdd.pc || 0,
            meetingRoomState: 0,
      ...
      Modify(record) {
        ...
        // console.log('121212121222222222222')
        // console.log(record)
        this.formModify.meetingroomId = record.meetingroomId
        this.formModify.meetingRoomSize = record.meetingRoomSize
      ...
      onSubmitModify() {
        if (this.checkedValuesData) {
          this.checkedValuesData.map(item => {
            if (item == '白板') {
              this.formModify.whiteboard = 1
            }
            if (item == '黑板') {
              this.formModify.blackboard = 1
            }
            if (item == '投影仪') {
              this.formModify.projector = 1
            }
            if (item == '演示电脑') {
              this.formModify.pc = 1
            }
          })
        }
        console.log(this.formModify.condition)
        this.handleFormAddData01.push({
          meetingroomId: this.formModify.meetingroomId,
          whiteboard: this.formModify.whiteboard || 0,
          blackboard: this.formModify.blackboard || 0,
          number: this.formModify.number,
          projector: this.formModify.projector || 0,
          pc: this.formModify.pc || 0,
          meetingRoomState: 0,
          meetingRoomStateName: null,
          placeName: this.formModify.placeName,
          meetingRoomSize: this.formModify.meetingRoomSize,
          state: this.radioRadio
        })
        console.log(formModify.state)
        console.log('852585222')
        console.log(this.radioRadio)
        return false
        // console.log(this.handleFormAddData01)
        updateMeetingRoom(this.handleFormAddData01[0])
          .then(response => {
            if (response.code == 200) {
      ...
      handlerTableData() {
        ...
        this.dataSource.map(item => {
          ...
          // console.log(this.handlerCondition)
